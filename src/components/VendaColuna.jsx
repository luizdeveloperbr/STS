    ...
    return (<>
        {!prop.venda.confirmado ?
            <div className="flex justify-start" id="e">
                <Formik initialValues={{ tipo: "R", banco: "", valorVenda: "", quantidade: "", confirmado: true }}
                    onSubmit={handleSubmitValue}>{({ values, handleSubmit }) => (
                        <>
                            <div className="footer w-[150px] pt-2">
                                <DataParsed timestamp={prop.venda.datetime} format="DD/MM/YYYY" />
                            </div>
                            <div className="w-[150px] pt-2 border border-black text-center">{prop.venda.userID}</div>
                            <div className="w-[150px] pt-2 border border-black text-center">{prop.venda.userName}</div>
                            <Form className="flex">
                                ...
                                <div className="border border-black text-center">
                                    <Field name="quantidade" className="form-input max-w-[80px]" type="number" placeholder="Qnt." ></Field>
                                </div>
                                <div className="border border-black text-center">
                                    <Field name="valorVenda" className="form-input max-w-[100px]" type="number" placeholder="R$ 100.00" ></Field>
                                </div>
                                <div className="w-[110px] pt-2 border border-black text-center"><Real valor={values.quantidade * prop.venda.custoUnitario} /></div>
                                <div className="w-[110px] pt-2 border border-black text-center"><Real valor={values.valorVenda - (values.quantidade * prop.venda.custoUnitario)} /></div>
                                <div className="w-[110px] pt-2 border border-black text-center" >
                                    {values.banco && values.quantidade && values.valorVenda ?
                                        <input className="scale-150" type="checkbox" onInput={handleSubmit} /> :
                                        <input className="scale-150" type="checkbox" disabled />
                                    }
                                </div>
                                <div className="w-[110px] pt-1 border border-black text-center">
                                    <a className="button mx-1 hover:cursor-pointer bg-blue-600" onClick={() => deletarVenda(prop.venda.id)}>E</a>
                                    <a className="button mx-1 hover:cursor-pointer" onClick={() => deletarVenda(prop.venda.id)}>X</a>
                                </div>
                            </Form>
                        </>
                    )}</Formik>
