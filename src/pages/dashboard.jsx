...
import ptBR from "../utils/pt-br"; import { Form, Formik, Field } from "formik";
...
function Dashboard() {
  ...
  async function listarVendas(id, coluna, valor, setState) {
    let colecao
    if (valor == "") {
  ...
  async function novoRegistro(values) {
    ...
    listarVendas(user.uid, "mes", "", setListaDeVendas)
  }
  ...
  return (
    <div id="main" className="p-4">
      <nav className="font-bold pl-2 min-h-[40px] border border-black rounded mb-2 flex flex-grow place-items-center bg-slate-200">
        ...
        <div>
          <a className="ml-2 button hover:cursor-pointer" onClick={handleSignOut} >{user.email}</a>
        </div>
      </nav>
          ...
          ({ isSubmitting }) => (
            ...
            <Form className="mx-auto flex overflow-x-auto">
              <label className="m-1 grid text-sm" htmlFor="datetime">
                Data
                <Field type="date" min={moment().format("YYYY-MM-DD")} name="datetime" className="form-input" required />
              </label>
              ...
              <label className="m-1 grid text-sm" htmlFor="custoUnitario">
                Custo Unitario
                <Field required type="number" className="form-input w-[90px]" name="custoUnitario" placeholder="R$ X,XX" />
              </label>
              <div className="self-end pb-2">
                <button type="submit" className="button disabled:text-red-600" disabled={isSubmitting}>Adicionar</button>
                <button type="reset" className="button mx-1">Limpar</button>
              </div>
            </Form>
          )
        }
      </Formik>
      <TabelaVendas vendas={listaDeVendas} />
